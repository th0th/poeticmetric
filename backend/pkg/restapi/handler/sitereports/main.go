...
// ReadSiteUTMCampaignReport godoc
// @Description Read UTM campaign report for a site.
// @Param filters query poeticmetric.SiteReportFilters true "Filters"
// @Param cursor query string false "Pagination cursor"
// @Router /site-reports/utm-campaign [get]
// @Security UserAccessTokenAuthentication
// @Success 200 {array} poeticmetric.SiteUTMCampaignReport
// @Summary Read UTM campaign report
// @Tags site-reports
func (h *Handler) ReadSiteUTMCampaignReport(w http.ResponseWriter, r *http.Request) {
	filters := middleware.GetSiteReportFilters(r)

	paginationCursor, err := getPaginationCursor[poeticmetric.SiteUTMCampaignReportPaginationCursor](r)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}

	report, err := h.siteService.ReadSiteUTMCampaignReport(r.Context(), filters, paginationCursor)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}
...
// @Param filters query poeticmetric.SiteReportFilters true "Filters"
// @Param cursor query string false "Pagination cursor"
// @Router /site-reports/utm-medium [get]
// @Security UserAccessTokenAuthentication
// @Success 200 {array} poeticmetric.SiteUTMMediumReport
// @Summary Read UTM medium report
// @Tags site-reports
func (h *Handler) ReadSiteUTMMediumReport(w http.ResponseWriter, r *http.Request) {
	filters := middleware.GetSiteReportFilters(r)

	paginationCursor, err := getPaginationCursor[poeticmetric.SiteUTMMediumReportPaginationCursor](r)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}

	report, err := h.siteService.ReadSiteUTMMediumReport(r.Context(), filters, paginationCursor)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}
...
// ReadSiteUTMSourceReport godoc
// @Description Read UTM source report for a site.
// @Param filters query poeticmetric.SiteReportFilters true "Filters"
// @Param cursor query string false "Pagination cursor"
// @Router /site-reports/utm-source [get]
// @Security UserAccessTokenAuthentication
// @Success 200 {array} poeticmetric.SiteUTMSourceReport
// @Summary Read UTM source report
// @Tags site-reports
func (h *Handler) ReadSiteUTMSourceReport(w http.ResponseWriter, r *http.Request) {
	filters := middleware.GetSiteReportFilters(r)

	paginationCursor, err := getPaginationCursor[poeticmetric.SiteUTMSourceReportPaginationCursor](r)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}

	report, err := h.siteService.ReadSiteUTMSourceReport(r.Context(), filters, paginationCursor)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}
...
// ReadSiteUTMTermReport godoc
// @Description Read UTM term report for a site.
// @Param filters query poeticmetric.SiteReportFilters true "Filters"
// @Param cursor query string false "Pagination cursor"
// @Router /site-reports/utm-term [get]
// @Security UserAccessTokenAuthentication
// @Success 200 {array} poeticmetric.SiteUTMTermReport
// @Summary Read UTM term report
// @Tags site-reports
func (h *Handler) ReadSiteUTMTermReport(w http.ResponseWriter, r *http.Request) {
	filters := middleware.GetSiteReportFilters(r)

	paginationCursor, err := getPaginationCursor[poeticmetric.SiteUTMTermReportPaginationCursor](r)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}

	report, err := h.siteService.ReadSiteUTMTermReport(r.Context(), filters, paginationCursor)
	if err != nil {
		h.responder.Error(w, errors.Wrap(err, 0))
		return
	}
