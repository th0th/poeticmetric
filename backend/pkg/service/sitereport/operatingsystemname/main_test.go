...
func TestGet(t *testing.T) {
	...
	_ = dp.WithPostgresTransaction(func(dp2 *depot.Depot) error {
		...
		testData := []*TestDatum{
			{OperatingSystemName: nil, VisitorCount: uint64(gofakeit.IntRange(1, 100))},
		}
		...
		for testDatumIndex := 0; testDatumIndex < 12; testDatumIndex += 1 {
			...
			testData = append(testData, &TestDatum{
				OperatingSystemName: pointer.Get(fmt.Sprintf("operating-system-%d", testDatumIndex+1)),
		...
		for _, testDatum := range testData {
			...
			for visitorIndex = 0; visitorIndex < testDatum.VisitorCount; visitorIndex += 1 {
				...
				for viewIndex := 0; viewIndex < viewCount; viewIndex += 1 {
					...
					if testDatum.OperatingSystemName != nil {
						operatingSystemName = testDatum.OperatingSystemName
					}
		...
		expectedReport := &Report{
			PaginationCursor: &PaginationCursor{
				OperatingSystemName: *filteredTestData[9].OperatingSystemName,
