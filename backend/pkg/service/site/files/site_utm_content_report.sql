...
WHERE
  if(
    isNull(@paginationVisitorCount) AND isNull(@paginationUTMContent),
    TRUE,
    visitor_count < @paginationVisitorCount OR (visitor_count = @paginationVisitorCount AND utm_content < @paginationUTMContent)
  )
