...
WHERE
  if(
    isNull(@paginationVisitorCount) AND isNull(@paginationUTMTerm),
    TRUE,
    visitor_count < @paginationVisitorCount OR (visitor_count = @paginationVisitorCount AND utm_term < @paginationUTMTerm)
  )
