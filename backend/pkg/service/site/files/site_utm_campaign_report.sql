...
WHERE
  if(
    isNull(@paginationVisitorCount) AND isNull(@paginationUTMCampaign),
    TRUE,
    visitor_count < @paginationVisitorCount OR (visitor_count = @paginationVisitorCount AND utm_campaign < @paginationUTMCampaign)
  )
