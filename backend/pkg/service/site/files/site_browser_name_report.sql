...
WHERE
  if(
    isNull(@paginationVisitorCount) AND isNull(@paginationBrowserName),
    TRUE,
    visitor_count < @paginationVisitorCount OR (visitor_count = @paginationVisitorCount AND browser_name < @paginationBrowserName)
  )
