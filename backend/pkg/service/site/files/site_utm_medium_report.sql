...
WHERE
  if(
    isNull(@paginationVisitorCount) AND isNull(@paginationUTMMedium),
    TRUE,
    visitor_count < @paginationVisitorCount OR (visitor_count = @paginationVisitorCount AND utm_medium < @paginationUTMMedium)
  )
