...
WHERE
  if(
    isNull(@paginationVisitorCount) AND isNull(@paginationBrowserVersion),
    TRUE,
    visitor_count < @paginationVisitorCount OR (visitor_count = @paginationVisitorCount AND browser_version < @paginationBrowserVersion)
  )
