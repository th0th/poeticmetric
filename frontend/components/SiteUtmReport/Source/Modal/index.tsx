...
export function Modal() {
  ...
  const show = useMemo<BsModalProps["show"]>(() => router.query.detail === "utm-source", [router.query.detail]);
  ...
  const { data: rawData, isValidating, setSize } = useSiteUtmSourceReport();

  const data = useMemo<Array<HydratedSiteUtmSourceDatum>>(() => {
    if (rawData === undefined) {
      return [];
    }

    return rawData.reduce<Array<HydratedSiteUtmSourceDatum>>((a, v) => [...a, ...v.data], []);
  }, [rawData]);
  ...
  return (
    <BsModal onHide={onHide} show={show}>
      <BsModal.Header closeButton>
        <BsModal.Title>UTM source</BsModal.Title>
      </BsModal.Header>
      ...
      <BsModal.Body>
        <Table borderless className="fs-sm table-layout-fixed" hover responsive striped>
          <thead>
            <tr>
              <th className="w-8rem">UTM source</th>
