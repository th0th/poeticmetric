...
export function Modal() {
  ...
  const show = useMemo<BsModalProps["show"]>(() => router.query.detail === "utm-term", [router.query.detail]);
  ...
  const { data: rawData, isValidating, setSize } = useSiteUtmTermReport();

  const data = useMemo<Array<HydratedSiteUtmTermDatum>>(() => {
    if (rawData === undefined) {
      return [];
    }

    return rawData.reduce<Array<HydratedSiteUtmTermDatum>>((a, v) => [...a, ...v.data], []);
  }, [rawData]);
  ...
  return (
    <BsModal onHide={onHide} show={show}>
      <BsModal.Header closeButton>
        <BsModal.Title>UTM term</BsModal.Title>
      </BsModal.Header>
      ...
      <BsModal.Body>
        <Table borderless className="fs-sm table-layout-fixed" hover responsive striped>
          <thead>
            <tr>
              <th className="w-8rem">UTM term</th>
