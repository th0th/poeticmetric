...
export function Modal() {
  ...
  const show = useMemo<BsModalProps["show"]>(() => router.query.detail === "utm-medium", [router.query.detail]);
  ...
  const { data: rawData, isValidating, setSize } = useSiteUtmMediumReport();

  const data = useMemo<Array<HydratedSiteUtmMediumDatum>>(() => {
    if (rawData === undefined) {
      return [];
    }

    return rawData.reduce<Array<HydratedSiteUtmMediumDatum>>((a, v) => [...a, ...v.data], []);
  }, [rawData]);
  ...
  return (
    <BsModal onHide={onHide} show={show}>
      <BsModal.Header closeButton>
        <BsModal.Title>UTM medium</BsModal.Title>
      </BsModal.Header>
      ...
      <BsModal.Body>
        <Table borderless className="fs-sm table-layout-fixed" hover responsive striped>
          <thead>
            <tr>
              <th className="w-8rem">UTM medium</th>
