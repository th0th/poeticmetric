...
export function Modal() {
  ...
  const show = useMemo<BsModalProps["show"]>(() => router.query.detail === "operating-system-name", [router.query.detail]);
  ...
  const { data: rawData, isValidating, setSize } = useSiteOperatingSystemNameReport();

  const data = useMemo<Array<HydratedSiteOperatingSystemNameDatum>>(() => {
    if (rawData === undefined) {
      return [];
    }

    return rawData.reduce<Array<HydratedSiteOperatingSystemNameDatum>>((a, v) => [...a, ...v.data], []);
  }, [rawData]);
  ...
  return (
    <BsModal onHide={onHide} show={show}>
      <BsModal.Header closeButton>
        <BsModal.Title>Operating systems</BsModal.Title>
      </BsModal.Header>
      ...
      <BsModal.Body>
        <Table borderless className="fs-sm table-layout-fixed" hover responsive striped>
          <thead>
            <tr>
              <th className="w-10rem">Operating system</th>
                ...
                <td className="parent-d" colSpan={2}>
                  <div className="d-flex flex-row">
                    <Link
                      ...
                      href={{ pathname: router.pathname, query: { ...router.query, operatingSystemName: d.operatingSystemName } }}
                      scroll={false}
