...
type StateDatum = {
  datum: HydratedSiteBrowserNameDatum;
  height: number;
  width: number;
  x: number;
  y: number;
...
export function BaseChart({ parentHeight, parentWidth }: BaseChartProps) {
  ...
  const { data: rawData } = useSiteBrowserNameReport();

  const handleRectClick = useCallback(async (d: HydratedSiteBrowserNameDatum) => {
    await router.push({ pathname: router.pathname, query: { ...router.query, browserName: d.browserName } }, undefined, { scroll: false });
  }, [router]);
  ...
  const state = useMemo<State | null>(() => {
    ...
    const yDomain: State["yDomain"] = slicedData.map((d) => d.browserName);
