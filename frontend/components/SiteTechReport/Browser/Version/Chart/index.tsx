...
export function BaseChart({ parentHeight, parentWidth }: BaseChartProps) {
  ...
  const { data: rawData } = useSiteBrowserVersionReport();

  const handleRectClick = useCallback(async (d: HydratedSiteBrowserVersionDatum) => {
    await router.push({
      pathname: router.pathname,
      query: { ...router.query, browserVersion: d.browserVersion },
  ...
  const state = useMemo<State | null>(() => {
    ...
    const yDomain: State["yDomain"] = slicedData.map((d) => d.browserVersion);
      ...
      {tooltipOpen && tooltipData !== undefined ? (
        <ChartTooltip left={tooltipLeft} top={tooltipTop}>
          ...
          <div className="mt-2 text-center">
            <div className="fs-xs fw-medium">{tooltipData.datum.browserVersion}</div>
