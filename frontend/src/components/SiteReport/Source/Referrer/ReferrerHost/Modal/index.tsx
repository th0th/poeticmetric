...
export default function Modal() {
  ...
  const { data: report, isValidating, setSize } = useSiteReferrerHostReport();

  const data = useMemo<Array<HydratedSiteReferrerHostReportDatum>>(() => {
    if (report === undefined) {
      return [];
    }

    return report.reduce<Array<HydratedSiteReferrerHostReportDatum>>((a, v) => [...a, ...v.data], []);
  }, [report]);

  const hasMore = useMemo<boolean>(() => !!(report?.at(-1)?.paginationCursor), [report]);
  const isShown = useMemo(() => searchParams.get("detail") === "referrer-host", [searchParams]);
              ...
              <tr className="parent" key={d.referrerHost}>
                <td colSpan={2}>
                  <Link
                    ...
                    to={getUpdatedLocation(location, { search: { referrerHost: d.referrerHost } })}
                  >
