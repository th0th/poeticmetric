...
export default function Modal() {
  ...
  const data = useMemo<Array<HydratedSiteUTMMediumReportDatum>>(() => {
    if (report === undefined) {
      return [];
    }

    return report.reduce<Array<HydratedSiteUTMMediumReportDatum>>((a, v) => [...a, ...v.data], []);
  }, [report]);

  const hasMore = useMemo<boolean>(() => !!(report?.at(-1)?.paginationCursor), [report]);
  const isShown = useMemo(() => searchParams.get("detail") === "utm-medium", [searchParams]);
  ...
  return (
    <BaseModal centered onHide={hide} show={isShown} size="lg">
      <BaseModal.Header closeButton>
        <BaseModal.Title>UTM mediums</BaseModal.Title>
      </BaseModal.Header>
