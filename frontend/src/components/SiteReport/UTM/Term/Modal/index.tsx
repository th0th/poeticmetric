...
export default function Modal() {
  ...
  const { data: report, isValidating, setSize } = useSiteUTMTermReport();
  ...
  const data = useMemo<Array<HydratedSiteUTMTermReportDatum>>(() => {
    ...
    return report.reduce<Array<HydratedSiteUTMTermReportDatum>>((a, v) => [...a, ...v.data], []);
  }, [report]);

  const hasMore = useMemo<boolean>(() => !!(report?.at(-1)?.paginationCursor), [report]);
  const isShown = useMemo(() => searchParams.get("detail") === "utm-term", [searchParams]);
  ...
  return (
    <BaseModal centered onHide={hide} show={isShown} size="lg">
      <BaseModal.Header closeButton>
        <BaseModal.Title>UTM terms</BaseModal.Title>
      </BaseModal.Header>
              ...
              <tr className="parent" key={d.utmTerm}>
                <td colSpan={2}>
                  <Link
                    ...
                    title={d.utmTerm}
                    to={`${location}?${getUpdatedSearch(searchParams, { detail: null, utmTerm: d.utmTerm })}`}
                  >
