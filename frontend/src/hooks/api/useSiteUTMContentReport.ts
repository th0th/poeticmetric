...
import { hydrateSiteUTMContentReport } from "~/lib/api/sites";
...
export default function useSiteUTMContentReport(config?: Config): Response {
  ...
  const getKey = useCallback<SWRInfiniteKeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/utm-content?${urlSearchParams.toString()}`;
  }, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSiteUTMContentReport(data);
  }, [baseFetcher]);
