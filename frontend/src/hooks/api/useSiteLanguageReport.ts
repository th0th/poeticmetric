...
import { hydrateSiteLanguageReport } from "~/lib/api/sites";
...
export default function useSiteLanguageReport(config?: Config): Response {
  ...
  const getKey = useCallback<SWRInfiniteKeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/language?${urlSearchParams.toString()}`;
  }, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSiteLanguageReport(data);
  }, [baseFetcher]);
