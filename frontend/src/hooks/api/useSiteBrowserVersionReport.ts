...
import { hydrateSiteBrowserVersionReport } from "~/lib/api/sites";
...
export default function useSiteBrowserVersionReport(config?: Config): Response {
  ...
  const getKey = useCallback<SWRInfiniteKeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/browser-version?${urlSearchParams.toString()}`;
  }, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSiteBrowserVersionReport(data);
  }, [baseFetcher]);
