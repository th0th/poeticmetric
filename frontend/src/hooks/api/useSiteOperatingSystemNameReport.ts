...
import { hydrateSiteOperatingSystemNameReport } from "~/lib/api/sites";
...
export default function useSiteOperatingSystemNameReport(config?: Config): Response {
  ...
  const getKey = useCallback<SWRInfiniteKeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/operating-system-name?${urlSearchParams.toString()}`;
  }, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSiteOperatingSystemNameReport(data);
  }, [baseFetcher]);
