...
import { hydrateSiteReferrerReport } from "~/lib/api/sites";

type Config = SWRInfiniteConfiguration<HydratedData, Error, BareFetcher<HydratedData>>;
type Data = SiteReferrerReport;
type HydratedData = HydratedSiteReferrerReport;
type Response = SWRInfiniteResponse<HydratedData, Error>;

export default function useSiteReferrerReport(config?: Config): Response {
  ...
  const getKey = useCallback<SWRInfiniteKeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/referrer?${urlSearchParams.toString()}`;
  }, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSiteReferrerReport(data);
  }, [baseFetcher]);
