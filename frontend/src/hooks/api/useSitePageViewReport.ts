...
import { hydrateSitePageViewReport } from "~/lib/api/sites";

type Config = SWRConfiguration<HydratedData, Error, BareFetcher<HydratedData>>;
type Data = SitePageViewReport;
type HydratedData = HydratedSitePageViewReport;
type Response = SWRResponse<HydratedData, Error>;

export default function useSitePageViewReport(config?: Config): Response {
  const { fetcher: baseFetcher } = useSWRConfig();
  const { filterQueryParams } = useSiteReportData();
  const path = useMemo<string | null>(() => `/site-reports/page-view${filterQueryParams}`, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSitePageViewReport(data);
  }, [baseFetcher]);
