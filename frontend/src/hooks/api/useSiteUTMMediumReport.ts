...
import { hydrateSiteUTMMediumReport } from "~/lib/api/sites";
...
export default function useSiteUTMMediumReport(config?: Config): Response {
  ...
  const getKey = useCallback<SWRInfiniteKeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/utm-medium?${urlSearchParams.toString()}`;
  }, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSiteUTMMediumReport(data);
  }, [baseFetcher]);
