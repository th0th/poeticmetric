...
import { hydrateSiteOperatingSystemVersionReport } from "~/lib/api/sites";
...
export default function useSiteOperatingSystemVersionReport(config?: Config): Response {
  ...
  const getKey = useCallback<SWRInfiniteKeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/operating-system-version?${urlSearchParams.toString()}`;
  }, [filterQueryParams]);
  ...
  const fetcher = useCallback<BareFetcher<HydratedData>>(async (...args) => {
    ...
    return hydrateSiteOperatingSystemVersionReport(data);
  }, [baseFetcher]);
