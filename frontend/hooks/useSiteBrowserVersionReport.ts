...
export function useSiteBrowserVersionReport(): HydratedSwrInfiniteResponse {
  ...
  const getKey = useCallback<KeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/browser-version?${stringify(queryParams)}`;
  ...
  const data = useMemo<HydratedSwrInfiniteResponse["data"] | undefined>(() => {
    ...
    return rawData.map(hydrateSiteBrowserVersionReport);
  }, [rawData]);
