...
export function useSiteUtmCampaignReport(): HydratedSwrInfiniteResponse {
  ...
  const getKey = useCallback<KeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/utm-campaign?${stringify(queryParams)}`;
  ...
  const data = useMemo<HydratedSwrInfiniteResponse["data"] | undefined>(() => {
    ...
    return rawData.map(hydrateSiteUtmCampaignReport);
  }, [rawData]);
