...
export function useSiteBrowserNameReport(): HydratedSwrInfiniteResponse {
  ...
  const getKey = useCallback<KeyLoader>((index, previousPageData) => {
    ...
    return `/site-reports/browser-name?${stringify(queryParams)}`;
  ...
  const data = useMemo<HydratedSwrInfiniteResponse["data"] | undefined>(() => {
    ...
    return rawData.map(hydrateSiteBrowserNameReport);
  }, [rawData]);
